...
export default function Home() {
	...
	async function fetchData() {
		setLoading(true)
		try {
			const response = await fetch(`/api/appointments/node1?page=${currentPage}`)
	...
	const searchAppointment = async (event) => {
		...
		try {
			const response = await fetch(
				`/api/appointments/node1?page=${currentPage}&action=search&column=${column}&searchValue=${searchValue}`,
			)
			const data = await response.json()
			setAppointments(data)
			setLoading(false)
				...
				<Card className="md:w-2/3 min-w-full h-full p-6 text-black rounded-[1rem] shadow-lg gap-4 justify-between flex flex-col border border-primary">
					...
					<Card className="flex w-full h-full rounded-[1rem] border border-primary overflow-hidden">
						{loading ? (
							...
							<AppointmentTable appointments={appointments} node={'node1'} />
						)}
					</Card>
				</Card>
			</div>
